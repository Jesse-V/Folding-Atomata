...
inline bool SwitchArg::processArg(int *i, std::vector<std::string>& args)
{
	...
	if ( argMatches( args[(std::size_t)*i] ) )
	{
		commonProcessing();
	...
	else if ( combinedSwitchesMatch( args[(std::size_t)*i] ) )
	{
		// check again to ensure we don't misinterpret
		// this as a MultiSwitchArg
		if ( combinedSwitchesMatch( args[(std::size_t)*i] ) )
		...
		return lastCombined( args[(std::size_t)*i] );
	}
	else
		return false;
}
